...
async function getDashboardMaster(req, res) {
  try {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      pTurnos.push(db.Fa13.count({ where: { createdAt: { [Op.gte]: x1i, [Op.lte]: x1f } }, distinct: true, col: "serial" }).then(c => ({ turno: 1, dia: i, count: c })));
      pTurnos.push(db.Fa13.count({ where: { createdAt: { [Op.gte]: x2i, [Op.lte]: x2f } }, distinct: true, col: "serial" }).then(c => ({ turno: 2, dia: i, count: c })));
      pTurnos.push(db.Fa13.count({ where: { createdAt: { [Op.gte]: x3i, [Op.lte]: x3f } }, distinct: true, col: "serial" }).then(c => ({ turno: 3, dia: i, count: c })));
    }
