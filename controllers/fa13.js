...
// ── NUEVO ENDPOINT MAESTRO PARA FA-13 ──
async function getDashboardMaster(req, res) {
  try {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      pTurnos.push(db.Fa13.count({ where: { createdAt: { [Op.gte]: t1Inicio, [Op.lt]: t1Fin } }, distinct: true, col: "serial" }).then(c => ({ turno: 1, dia: i, count: c })));
      pTurnos.push(db.Fa13.count({ where: { createdAt: { [Op.gte]: t2Inicio, [Op.lt]: t2Fin } }, distinct: true, col: "serial" }).then(c => ({ turno: 2, dia: i, count: c })));
      pTurnos.push(db.Fa13.count({ where: { createdAt: { [Op.gte]: t3Inicio, [Op.lt]: t3Fin } }, distinct: true, col: "serial" }).then(c => ({ turno: 3, dia: i, count: c })));
    }
    ...
    console.error("Error en getDashboardMaster (FA13):", error);
