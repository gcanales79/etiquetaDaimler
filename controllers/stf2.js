...
async function addSerial(req, res) {
  ...
  try {
    ...
    const partResponse = await db.Numeropt.findOne({
      where: {
        linea: "STF-2", // Cambiado para Stf2
    ...
    if (!partResponse) {
      return res.send({
        code: "400",
        message: "El número de parte no está dado de alta en la línea STF-2",
      });
    }

    try {
      // 3. Intento de creación del registro
      const serialStored = await db.Stf2.create({
        serial: serial,
        numero_parte: numero_parte,
      ...
      return res.send({
        ...
        message: "Etiqueta correcta (STF-2)",
      });
      ...
      if (err.name === 'SequelizeUniqueConstraintError') {
        ...
        return res.send({
          code: "400",
          message: "Número de serie repetido en STF-2",
        });
      }
    ...
    console.error("Error en Stf2 addSerial:", error);
...
async function productionPerHour(req, res) {
  try {
    ...
    const data = await db.Stf2.findAndCountAll({
      where: {
        createdAt: {
          [Op.gte]: fechainicial,
          [Op.lte]: fechafinal,
...
async function getDashboardMaster(req, res) {
  try {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      pTurnos.push(db.Stf2.count({ where: { createdAt: { [Op.gte]: t1Inicio, [Op.lt]: t1Fin } }, distinct: true, col: "serial" }).then(c => ({ turno: 1, dia: i, count: c })));
      pTurnos.push(db.Stf2.count({ where: { createdAt: { [Op.gte]: t2Inicio, [Op.lt]: t2Fin } }, distinct: true, col: "serial" }).then(c => ({ turno: 2, dia: i, count: c })));
      pTurnos.push(db.Stf2.count({ where: { createdAt: { [Op.gte]: t3Inicio, [Op.lt]: t3Fin } }, distinct: true, col: "serial" }).then(c => ({ turno: 3, dia: i, count: c })));
    }
    ...
    console.error("Error en getDashboardMaster (STF-2):", error);
