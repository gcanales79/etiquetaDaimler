...
module.exports = function(app) {
  ...
  app.get("/api/daimler/dashboard-master", async (req, res) => {
    try {
      ...
      // 1. Últimas 6 (Solo de producción)
      ...
      for (let i = 0; i < 9; i++) {
        ...
        pHoras.push(
          ...
          }).then(count => ({ fecha: inicioMty.format("X"), producidas: count }))
        );
      }

      // 3. Producción por Turnos
      let pTurnos = [];
      ...
      // 4. Producción por Semanas (Ajustado a Domingo 23:00)
      let pSemanas = [];
      for (let i = 9; i >= 0; i--) {
        ...
        pSemanas.push(
          db.Daimler.count({
            ...
            distinct: true, col: "serial"
          }).then(count => ({ semana: numeroDeSemana, valor: count }))
        );
      }
