$(document).ready(function () {
  ...
  // IMPORTANTE: Asegúrate de crear la ruta /api/fa2/dashboard-master en tu backend
  function actualizarTodosLosDashboards() {
    $.get("/api/fa2/dashboard-master")
      .then((respuesta) => {
      ...
      .fail((error) => {
        console.error("Error al cargar el Dashboard Maestro de FA-2", error);
      });
  }
  ...
  $btnSubmit.on("click", function (event) {
    ...
    $.post("/api/fa2/serial", { serial: nuevoSerial }).then((data) => {
      switch (data.code) {
        case "200":
