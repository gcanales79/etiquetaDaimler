$(document).ready(function() {
  ...
  $("#submit").on("click", function(event) {
    ...
    $.post("/api/serial", { serial: nuevoSerial }).then((data) => {
      switch (data.code) {
  ...
  function getLast6() {
    ...
    $.get("/api/all/tabla/seisetiquetas", function(datos) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        $("#tablaDe6").prepend("<tr><th scope='row'>" + data[i].serial + "</th><td><span class=" + icon + "></span></td><td>" + fecha + "</td></tr>");
      }
  ...
  function produccionPorhora() {
    ...
    for (let i = 0; i < 9; i++) {
      ...
      $.get("/produccionhora/" + fechainicial + "/" + fechafinal, function(datos) {
  ...
  function produccionTurnos() {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      $.when(
        $.get("/produccionhora/" + x1i + "/" + x1f, function(d) { d1.splice(i, 0, d.data.count); }),
        $.get("/produccionhora/" + x2i + "/" + x2f, function(d) { d2.splice(i, 0, d.data.count); }),
        $.get("/produccionhora/" + x3i + "/" + x3f, function(d) { d3.splice(i, 0, d.data.count); })
  ...
  function produccionPorsemana() {
    ...
    for (let i = 9; i >= 0; i--) {
      ...
      $.when(
        $.get("/produccionhora/" + fechainicial + "/" + fechafinal, function(info) {
