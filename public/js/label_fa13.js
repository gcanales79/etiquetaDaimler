$(document).ready(function() {
  ...
  $("#submit").on("click", function(event) {
    ...
    $.post("/api/fa13/serial", { serial: nuevoSerial }).then((data) => {
      switch (data.code) {
  ...
  function produccionPorhora() {
    ...
    for (let i = 0; i < 9; i++) {
      ...
      $.get("/fa13/produccionhora/" + fechainicial + "/" + fechafinal, function(datos) {
  ...
  function produccionTurnos() {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      $.when(
        $.get("/fa13/produccionhora/" + x1i + "/" + x1f, function(d) { d1.splice(i, 0, d.data.count); }),
        $.get("/fa13/produccionhora/" + x2i + "/" + x2f, function(d) { d2.splice(i, 0, d.data.count); }),
        $.get("/fa13/produccionhora/" + x3i + "/" + x3f, function(d) { d3.splice(i, 0, d.data.count); })
  ...
  function produccionPorsemana() {
    ...
    for (let i = 9; i >= 0; i--) {
      ...
      $.when(
        $.get("/fa13/produccionhora/" + fechainicial + "/" + fechafinal, function(info) {
