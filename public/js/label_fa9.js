$(document).ready(function () {
  // 1. Caché de selectores: El navegador te lo agradecerá.
  ...
  function actualizarDashboards() {
    // 1 sola petición apuntando a FA-9
    $.get("/api/fa9/dashboard-master")
      .then((respuesta) => {
      ...
      .fail((error) => {
        console.error("Error al cargar el Dashboard Maestro FA-9", error);
      });
  }
  ...
  $btnSubmit.on("click", function (event) {
    ...
    const nuevoSerial = $serialInput.val().trim();
    localStorage.setItem("ultimaEtiqueta", nuevoSerial);

    const usuario = document.cookie.replace(/(?:(?:^|.*;\s*)usuario\s*\=\s*([^;]*).*$)|\^.*$/, "$1");
    localStorage.setItem("usuario", usuario);

    $.post("/api/fa9/serial", { serial: nuevoSerial })
      .then((data) => {
        switch (data.code) {
          case "200":
      ...
      .always(() => {
        // Ejecutamos la actualización de tableros 1 sola vez al terminar la petición
        actualizarDashboards();
      });
  });
